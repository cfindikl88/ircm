...
# ── 1. ORTAM ─────────────────────────────────────────────────────────────────
print("=" * 68)
print("  ICCKV-SENTINEL v3.2 — Trendyol-LLM-8B-T1 Full Pipeline")
print("=" * 68)
...
# Trendyol-LLM-8B-T1 — Apache-2.0, gated değil, direkt indirilir
CANDIDATE_MODELS = [
...
for mid in CANDIDATE_MODELS:
    try:
        ...
        # Qwen3 BF16; flash_attention_2 T4'te desteklenmez → eager
        model = AutoModelForCausalLM.from_pretrained(
            mid,
            torch_dtype=torch.bfloat16,
            device_map="auto",
            attn_implementation="eager",   # T4 flash_attn2 desteklemiyor
        )
        MODEL_ID = mid
...
if model is None:
    raise RuntimeError(
        "Model yüklenemedi.\n"
        "Trendyol/Trendyol-LLM-8B-T1 Apache-2.0 lisanslıdır, HF token gerekmez.\n"
        "İnternet bağlantısını veya disk alanını kontrol edin (model ~16 GB)."
    )
...
_A = """Transformer mimarisi, tüm diziler boyunca token ilişkilerini ağırlıklandırmaya
olanak tanıyan öz-dikkat mekanizmalarını tanıtarak doğal dil işlemede devrim yarattı.
Temel içgörü, sorgular ve anahtarlardan hesaplanan dikkat ağırlıklarının dilsel
bağımlılıkları yinelemeli yaklaşımlardan çok daha etkili biçimde yakaladığıydı.
Modern büyük dil modelleri düzinelerce transformer katmanını üst üste koyarak çok
başlıklı dikkat ve ileri beslemeli ağlar aracılığıyla bağlamsal temsilleri rafine eder.
Gruplandırılmış sorgu dikkati gibi verimlilik iyileştirmeleri, dizi uzunluğuyla
doğrusal olarak büyüyen ve uzun bağlamlı çıkarımda bir darboğaz haline gelen
anahtar-değer önbelleğinin bellek ayak izini azaltır. Önem derecesi düşük tokenların
atılması, depolanan tensörlerin nicemlenmesi ve dikkat matrislerinin düşük sıralı
yaklaşımları dahil çeşitli önbellek sıkıştırma stratejileri önerilmiştir. Sıkıştırma
oranı ile dil kalitesi arasındaki dengeye ilişkin merkezi zorluk, model tutarlılığını
ve çıkış kalitesini korumak için her pratik kullanımda dikkatle yönetilmelidir."""

_B = """Bilgi teorisi, karşılıklı bilgi ve entropi ölçüleri aracılığıyla rastgele değişkenler
arasındaki ilişkileri nicelleştirmek için katı bir çerçeve sunar. Wyner-Ziv teoremi,
klasik hız-bozulma teorisini iletilmeksizin kod çözücüde ilgili yan bilginin mevcut
olduğu senaryolara genişletir. Bu durum, alıcının istatistiksel olarak ilişkili bir
sinyale zaten sahip olduğu dağıtılmış sıkıştırma sistemleri için derin çıkarımlar
taşır. Sinir ağı çıkarımı bağlamında dikkat katmanlarındaki anahtar ve sorgu
tensörleri, ağ derinliği boyunca bağlamsal entegrasyonun biriktiği derin katmanlarda
giderek güçlenen ölçülebilir istatistiksel bağımlılık sergiler. Bu bağımlılığın doğrusal
regresyon yoluyla kullanılması, alıcının anahtar tensör varyansının önemli bir
bölümünü yalnızca sorgudan tahmin etmesine olanak tanır ve bu durum orantısız sinyal
kalitesi veya görev performansı kayıpları olmaksızın önemli bit hızı tasarruflarını
mümkün kılar."""

_C = """E-ticaret platformlarında ürün kataloğu zenginleştirme, yapay zeka modellerinin
büyük ölçekli metin verilerinden anahtar-değer çiftlerini çıkarmasını gerektirir.
Moda, elektronik ve ev dekorasyonu gibi kategorilerde binlerce ürün özelliğinin
doğru ve tutarlı biçimde etiketlenmesi, arama alaka düzeyini ve dönüşüm oranlarını
doğrudan etkiler. Büyük dil modelleri bu görevde güçlü performans sergilemektedir
ancak çıkarım gecikmesi ve bellek kullanımı üretim ortamlarında kritik kısıtlamalardır.
Anahtar-değer önbellek sıkıştırması bu kısıtlamaları hafifletmenin doğal bir yolunu
sunar çünkü katalog zenginleştirme görevleri tipik olarak benzer şablonlarla uzun
bağlamlar içerir ve bu da önbellek tensörleri arasında yüksek istatistiksel korelasyona
yol açar. IRCM gibi bilgi-teorik sıkıştırma yöntemlerinin bu spesifik kullanım durumuna
uyarlanması, hem hesaplama maliyetlerini hem de çıkarım gecikmesini önemli ölçüde
azaltırken katalog kalitesini koruma potansiyeli taşımaktadır."""
...
fig.suptitle(
    f"ICCKV-SENTINEL v3.2: IRCM vs Token Eviction — Sparsity–Quality Pareto Curve\n"
...
plt.tight_layout()
plot_path = os.path.join(OUTPUT_DIR, "icckv_trendyol_sparsity_curve.png")
...
json_path = os.path.join(OUTPUT_DIR, "icckv_trendyol_results.json")
