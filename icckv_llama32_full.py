"""
╔══════════════════════════════════════════════════════════════════════════╗
║  ICCKV-SENTINEL v3.0 — Llama 3.2-3B Full Pipeline                     ║
...
CANDIDATE_MODELS = [
    ...
    "unsloth/Llama-3.2-3B-Instruct",          # public mirror
    "bartowski/Llama-3.2-3B-Instruct-GGUF",   # son çare gguf değil ama denenebilir
]
...
if model is None:
    raise RuntimeError(
        ...
        "veya 'meta-llama/Llama-3.2-3B' için Hugging Face'de erişim iste."
    )
...
fig.suptitle(
    f"ICCKV-SENTINEL v3.0: IRCM vs Token Eviction — Sparsity–Quality Pareto Curve\n"
...
plt.tight_layout()
plot_path = os.path.join(OUTPUT_DIR, "icckv_llama32_sparsity_curve.png")
...
json_path = os.path.join(OUTPUT_DIR, "icckv_llama32_results.json")
